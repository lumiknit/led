...
class EditContext {
  ...
  deleteSelection() {
    ...
    if(startLine === endLine) {
      ...
      this.lines[startLine] = line.substring(0, startCol) + line.substring(endCol);
  ...
  delete(n: number) {
    // If selection is not empty, delete selection first
    if(this.isSelectionEmpty()) {
      let cur = this.getMovedCursor(this.selectionStart, -n);
      if(n > 0) {
  ...
  renderLine(index: number) {
    ...
    let r = (<div key={this.lineIDs[index]} className="code-line">
      {indent > 0 ? <span className="code-indent">{'-'.repeat(indent)}</span> : null}
